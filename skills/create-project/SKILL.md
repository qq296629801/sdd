...
7. 明确 IIDP 契约：`appName`、`appPkg`、`moduleName`、`model_name`、视图 key、菜单 key、服务、入参、权限码、前端实现分支、节点 id、数据源。
...
| SDD 流程、需求规格、Critique、Plan Review Gate、Blueprint、计划、任务、验收目录 | `references/sdd-workflow.md` |
...
| 前端交互设计规格、行为状态、响应式和可用性 | `references/sdd-frontend-interaction.md`，再按下方前端子 skill 路由规则执行 |
| JSON-RPC、Filter、按钮服务、节点属性契约 | `references/sdd-contracts.md` |
| 验证、失败处理、阶段复盘、Spec Sync 漂移检测、PR Bridge PR 描述生成、变更记录 | `references/sdd-validation.md` |
