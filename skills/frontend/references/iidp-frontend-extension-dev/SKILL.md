...
> 查阅 IIDP 框架机制、组件属性、扩展协议、工程配置等文档时，使用 [iidp-frontend-dev-manual](../iidp-frontend-dev-manual/SKILL.md)。该 skill 涵盖节点系统、数据源、属性绑定、选择器、数据联动、事件绑定、公共命令、视图行为协议、Hook 系统、扩展开发等核心机制的完整参考，以及所有组件的文档索引。需要原始文档细节时再按索引到 `iidpDoc/` 查找。
...
- **API 请求统一走 IIDP 数据源或 `window.Tech.httpMeta`**，禁止自行引入 axios、fetch 等请求库。使用 `window.Tech.httpMeta` 前必须先查阅 IIDP 文档确认其用法和参数格式，不要凭猜测编写。
...
当业务需求是**完全替换标准模板页面**（如主表格页需要整体重做）时：

- **首选 `replace` 替换主表格页节点**（如 `replace` 主表格的 `container_main` 或整个 grid 区域），用新的 `view` 完全接管。
